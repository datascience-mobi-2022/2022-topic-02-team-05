...
cancer as a list of string vectors.
...
was used to minimize computation time. **xxx Quelle: seurat package**
...
A neural network was used to predict the activity of the
...
were used to validate the precision of the network, obtaining a mean
squared error (MSE) value. For identification of the best initial
conditions, 25 different networks were generated each of them with two
hidden layers and different combinations of neurons per layer. For each
combination the MSE is calculated and the three combinations with the
lowest MSE are selected for selection of the best initial conditions
regarding the weights and biases. For each of the three networks 100
random initial conditions are tested, resulting in one singular network
with the lowest MSE.
