...
From the Molecular Signature Database (MSigDB) metabolic
pathways were selected [@msigdb]. First, they were compared to the given
...
lower variance than a certain threshold value are removed.
For the
...
calculated.
There were a few duplicates between the metabolic and
...
UMAP is performed with the package "umap" [@umap].
...
GSEA is performed with the package "fgsea" [@fgsea]. First, the log2FC is calculated for every gene of each each patient and
...
not need a reference data frame to compare to. 
Hence, there was no expression data provided for comparison in the TCGA analysis, GSVA was used. There are various solutions to perform GSVA, one of them is
performed by @GSVA by following those five steps.
For performing a GSVA, firstly the cumulative density distribution of a gene
