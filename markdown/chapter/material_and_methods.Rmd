...
based on the activity of the other pathways. Firstly, the correlation of
the pathways that are used for prediction is checked. Only pathways with
a low correlation were kept. In the next step, the variance is checked.
80% of the genes with low variance were omitted. For the regression
analysis only 20% of the pathways were used, to only use significant
pathways. The regression analysis was tested by xxx
...
the appendix (see table \@ref(tab:packagesused)). A neural network was
used to predict the activity of the REACTOME_INTERLEUKIN_36_PATHWAY
based on the activity of other pathways. Therefore, the network was
trained with the pathway activity of 45 xxx patients from the THCA data
for focused analysis. The other 15 patients were used to validate the
precision of the network, obtaining a mean squared error (MSE) value.
For identification of the best initial conditions, 25 different networks
were generated each of them with two hidden layers and different
combinations of neurons per layer. For each combination the MSE is
calculated and the three combinations with the lowest MSE are selected
for selection of the best initial conditions regarding the weights and
biases. For each of the three networks 100 random initial conditions are
tested, resulting in one singular network with the lowest MSE.
