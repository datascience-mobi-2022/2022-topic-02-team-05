...
plot(test, null.prediction, col='orange', main='Null model',pch=18, cex=1,
     ylab = 'prediction', xlab = 'true value',xlim=c(-1,0), ylim=c(-5,2))
abline(0,1,lwd=2)
...
predictions = matrix(c(nn.prediction, lm.prediction, lm.pca.prediction, null.prediction), ncol = 4,
                     dimnames = list(seq(1,15,1), c('nn','lm','lm.pca','null')))
...
#=> Alle Daten können als Normalverteilt angenommen werden

#Durchführen der F-tests jedes gegen jedes Modell
#Test ob x siginfikant BESSER als y
ftest = apply(residuals, 2, FUN = function(x){
  apply(residuals, 2,  FUN = function(y){
    return(var.test(x, y, alternative = 'less')$p.value)
  })
})
